...
if [ "$OPTION" = "--mount" ]; then
	...
	if mountpoint -q "/home/$SUDO_USER/.binds/$FITS_PATH"; then
		echo "Error: /home/$SUDO_USER/.binds/$FITS_PATH is already mounted; aborting to avoid cyclic mounts." | tee -a /var/log/pathfinder.log
		exit 1
    else
    # Verify that the provided sudo group matches the namespace
        if [ $FITS_GROUP != "$SUDO_GROUP" ]; then
            echo "Error: Provided sudo group '$SUDO_GROUP' does not match fits group '$FITS_GROUP'; aborting." | tee -a /var/log/pathfinder.log
            exit 1
        else
            ...
            echo "bindfs mount on /skadata/$FITS_PATH to /home/$SUDO_USER/.binds/$BIND_PATH with permissions 0700 for user $SUDO_USER" | tee -a /var/log/pathfinder.log
            mount --bind "/home/$SUDO_USER/.binds/$BIND_PATH/$FITS_FILE" "/home/$SUDO_USER/projects/$FITS_FILE"
            echo "Mounted /home/$SUDO_USER/.binds/$BIND_PATH/$FITS_FILE to /home/$SUDO_USER/projects/$FITS_FILE" | tee -a /var/log/pathfinder.log
        fi
	fi
    # Verify the mount was successful
    if mountpoint -q "/home/$SUDO_USER/projects/$FITS_FILE"; then
        echo "Mount verification successful: $FITS_FILE is mounted at /home/$SUDO_USER/projects/$FITS_FILE" | tee -a /var/log/pathfinder.log
    else
        echo "Error: Mount verification failed for $FITS_FILE at /home/$SUDO_USER/projects/$FITS_FILE" | tee -a /var/log/pathfinder.log
        exit 1
    fi
    ...
    echo "Unmounted $FITS_FILE from /home/$SUDO_USER/projects/$FITS_FILE" | tee -a /var/log/pathfinder.log
fi
